...
def get_empresa_alias(empresas):
	option_return = 0
	print("Selecciona una empresa:")
	for i in range(len(empresas)):
		print("	"+str(i)+") "+empresas[i]['name'])

	option = input("# Opción "+str(0)+"-"+str(len(empresas)-1)+": ")
	try:
		opt_int = int(option)
		
	except Exception as e:
		# print("\n \n ####################### ERROR: ",e)
		print("\n Elige una opción entre: "+str(0)+"-"+str(len(empresas)-1))
		option_return = get_empresa_alias(empresas)
	else:
		if (opt_int > -1) and (opt_int < len(empresas)):
			option_return = empresas[opt_int]
			print("Empresa:",option_return['name'])
		else:
			print(" \nElige una opción entre: "+str(0)+"-"+str(len(empresas)-1))
			option_return = get_empresa_alias(empresas)
	# print("option_return:",option_return)
	return option_return


def get_ingest_client_type(ingest_client_types):
	option_return = 0
	print("Selecciona una opción:")
	for i in range(len(ingest_client_types)):
		print("	"+str(i)+")",ingest_client_types[i])
	# print("# Opción "+str(0)+"-"+str(len(ingest_client_types)-1)+": ")
	option = input("# Opción "+str(0)+"-"+str(len(ingest_client_types)-1)+": ")
	try:
		opt_int = int(option)
		
	except Exception as e:
		# print("\n \n ####################### ERROR: ",e)
		print("\n Elige una opción entre: "+str(0)+"-"+str(len(ingest_client_types)-1))
		option_return = get_ingest_client_type(ingest_client_types)
	else:
		if(opt_int < len(ingest_client_types)):
			option_return = ingest_client_types[opt_int]
			print("Type:",option_return)
		else:
			print(" \nElige una opción entre: "+str(0)+"-"+str(len(ingest_client_types)-1))
			option_return = get_ingest_client_type(ingest_client_types)
	finally:
		# print("finally")
		pass
	# return ingest_client_types[option]
	return option_return




ingest_client_types = ["massive","local","remote"]
empresas = [{"alias":"M_DF","name":"Milenio DF"},{"alias":"MR_CDMX","name":"Milenio Radio CDMX"},{"alias":"MT_CDMX","name":"Medio Tiempo CDMX"},{"alias":"MR_MTY","name":"Milenio Radio Monterrey"},{"alias":"M_MTY","name":"Milenio Monterrey"},{"alias":"MM_MTY","name":"Multimedios Monterrey"}]
conf_file = "conf.json"
conf = load_conf(conf_file)
if(conf != False):

	conf_keys_required = ["istorage_api_servers"]
	...
	if(continue_runing):
		...
		client_name = empresa + "_" + tipo_ingesta[:3]+"-"+ip.replace(".","") + "-" + hex_mac_address + "-" + str(time_i())
